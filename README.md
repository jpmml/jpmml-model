...
The current version is **1.4.15** (7 September, 2019).
...
The resulting uber-JAR file `target/pmml-model-example-executable-1.4-SNAPSHOT.jar` contains the following command-line applications:
...
```
java -cp target/pmml-model-example-executable-1.4-SNAPSHOT.jar org.jpmml.model.ValidationExample --input model.pmml
```

Getting help:
```
java -cp target/pmml-model-example-executable-1.4-SNAPSHOT.jar <application class name> --help
```
...
For example:
```
java -Djavax.xml.bind.context.factory=org.eclipse.persistence.jaxb.JAXBContextFactory -cp target/pmml-model-example-executable-1.4-SNAPSHOT.jar ...
```
